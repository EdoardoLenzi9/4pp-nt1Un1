...
bool Append(struct List*lpList, char*lpAppended, struct TombStone* lpSymbol) {
	bool bSuccess = false;
	if (lpList != NULL) {
...
struct TombStone* GetSymbolID(struct SymbolTable*ctTable, char*lpItem) {
	struct TombStone* iReturnedClosure = NULL;
	if (ctTable != NULL && lpItem != NULL) {
		if (ctTable->lpSymbols != NULL) {
