...
int Evaluate(struct OperationNode *lpOperationNode) {
	...
	if (lpOperationNode!=NULL) {
		switch (lpOperationNode->iType) {
			...
			case ARITHMETIC_UNARY_MINUS:{iValue = -Evaluate(lpOperationNode->lpLeftOperationNode); break;}
...
void*Execute(struct OperationNode*lpCmd) {
	if (lpCmd!=NULL) {
		switch (lpCmd->iType) {
			...
			case PRINT_OP:{
				printf( "%d", Evaluate(lpCmd->lpLeftOperationNode));
				break;
			}
			case WHILE_OP:{
